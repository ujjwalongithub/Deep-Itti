...
from .common import conv3x3_block_itti_koch
...
class VGG(nn.Module):
    ...
    def __init__(self,
                 ...
                 in_size=(224, 224),
                 num_classes=1000,
                 itti_koch : bool=False):
        super(VGG, self).__init__()
        self.in_size = in_size
        self.num_classes = num_classes
        self.itti_koch = itti_koch

        self.features = nn.Sequential()
        for i, channels_per_stage in enumerate(channels):
            stage = nn.Sequential()
            for j, out_channels in enumerate(channels_per_stage):
                if not self.itti_koch:
                    ...
                    stage.add_module("unit{}".format(j + 1), conv3x3_block_itti_koch(
                        in_channels=in_channels,
                        out_channels=out_channels,
                        bias=bias,
                        use_bn=use_bn))
