...
from .itti_koch import IttiKochSaliency
...
class ConvBlock_itti_koch(nn.Module):
    """
    Standard convolution block with Batch normalization and activation.
    ...
    def __init__(self,
                 ...
                 bn_eps=1e-5,
                 activation=(lambda: nn.ReLU(inplace=True))):
        super(ConvBlock_itti_koch, self).__init__()
        self.activate = (activation is not None)
        self.use_bn = use_bn
        self.use_pad = (isinstance(padding, (list, tuple)) and (len(padding) == 4))
        self.sal_map = IttiKochSaliency()
...
def conv3x3_block_itti_koch(in_channels,
                  out_channels,
                  stride=1,
                  padding=1,
                  dilation=1,
...
def count_parameters(model):
    """
    Given a PyTorch model, computes the number of trainable and non-trainable
    parameters in the model
    :param model: A PyTorch model
    :return: A tuple (x,y,z) where x is the number of trainable parameters,
    y is the total number of parameters and z is the number of non-trainable
    parameters.
    """
    pytorch_total_params = sum(p.numel() for p in model.parameters())
    pytorch_trainable_params = sum(
        p.numel() for p in model.parameters() if p.requires_grad)
    return pytorch_trainable_params, pytorch_total_params, \
